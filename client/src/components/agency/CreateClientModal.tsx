...
export function CreateClientModal({ organizationId }: { organizationId: string }) {
  ...
  const modalId = `create-client-modal-${instanceId}`;
  const titleId = `create-client-title-${instanceId}`;
  ...
  return (
    <>
      ...
      {open ? (
        <div
          id={modalId}
          className="fixed inset-0 z-[90] size-full overflow-y-auto bg-neutral-950/50"
          role="dialog"
          aria-modal="true"
        ...
        >
          <div className="m-3 flex min-h-[calc(100%-1.5rem)] items-center justify-center sm:mx-auto sm:w-full sm:max-w-lg">
            <form
              onSubmit={submit}
              ...
              <div className="flex items-center justify-between border-b border-neutral-200 px-5 py-4 dark:border-neutral-800">
                <div>
                  ...
                  <p className="mt-1 text-sm text-neutral-500">
                    {t("This also creates the client access team.")}
                  </p>
                </div>
                <button
                  type="button"
