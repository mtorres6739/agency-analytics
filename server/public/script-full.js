...
(() => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        resolvedConfig = {
          ...
          identityResolutionEnabled: apiConfig.identityResolution?.enabled === true && scriptTag.getAttribute("data-identity-resolution") !== "false",
  ...
  var IdentityConsentManager = class {
    ...
    async send(granted, gpc = this.gpcEnabled()) {
    ...
    renderBanner() {
      ...
      root.style.cssText = "position:fixed;z-index:2147483647;left:16px;right:16px;bottom:16px;max-width:620px;margin:auto;padding:16px;border:1px solid #d4d4d4;border-radius:12px;background:#fff;color:#171717;box-shadow:0 12px 36px rgba(0,0,0,.18);font:14px/1.45 system-ui,sans-serif";
      const text = document.createElement("p");
      text.style.cssText = "margin:0 0 12px";
      text.textContent = "May we use optional identity matching to understand who visits this website? Regular privacy-friendly analytics works either way.";
      const actions = document.createElement("div");
      actions.style.cssText = "display:flex;gap:8px;flex-wrap:wrap";
      const accept = document.createElement("button");
      accept.type = "button";
      accept.textContent = "Allow identity matching";
      accept.style.cssText = "border:0;border-radius:8px;background:#0f766e;color:#fff;padding:9px 12px;font:600 14px system-ui;cursor:pointer";
      accept.addEventListener("click", () => void this.grant());
      const decline = document.createElement("button");
      decline.type = "button";
      decline.textContent = "Continue anonymously";
      decline.style.cssText = "border:1px solid #a3a3a3;border-radius:8px;background:#fff;color:#171717;padding:9px 12px;font:600 14px system-ui;cursor:pointer";
